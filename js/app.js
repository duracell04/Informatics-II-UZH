...
function orderedChildren(parent) {
  ...
  const orders = {
    sorting: ['counting','sort3','heapSort','quick','merge','insertion','selection','xsort','bubble','stackSort','pairSum','pairSumSorted'],
    graphs: ['graphTerminology','adjacency','graphState','bfs','dfs','cycle','allPaths','khop','edgeTypes','topologicalSort'],
    weightedGraphs: ['weightedGraph','mst','cutLightEdge','primJarnik','kruskal','priorityQueue','sssp','relaxation','bellmanFord','dagSSSP','dijkstra','negativeCycle','arbitrage'],
    dp: ['dpTemplate','memoization','matrixPath','palCuts','bigPlus','editDistance','maxProduct','nearestZero','matrixChain','lcs','coinChange','solutionReconstruction']
  };
  if (orders[parent.id]) {
    return kids.sort((a, b) => orderIndex(orders[parent.id], a.id) - orderIndex(orders[parent.id], b.id));
  }
  return kids;
}
...
function childSpread(count, depth = 2) {
  ...
  const extra = Math.min(Math.PI / 4.8, Math.max(0, count - 4) * Math.PI / 38);
  return base + extra;
}
...
function visible() {
  ...
  majors.forEach((m, i) => {
    ...
    kids.forEach((c, j) => {
      const local = kids.length === 1 ? 0 : (j / (kids.length - 1) - 0.5) * spread;
      const a = angle + local;
      const r = adaptiveRadius(r2Base, kids.length, j, subtreeSize(c), 2);
      const cp = polar(r, a);
      nodes.push({ ...c, x: cp.x, y: cp.y, depth: 2, angle: a, parentId: m.id });
      ...
      const r3Base = R3_BASE + Math.max(0, majorWeight - 8) * 13 + Math.max(0, kids.length - 6) * 30;
      gkids.forEach((g, k) => {
        const glocal = gkids.length === 1 ? 0 : (k / (gkids.length - 1) - 0.5) * gSpread;
        const ga = a + glocal;
        const gr = adaptiveRadius(r3Base, gkids.length, k, subtreeSize(g), 3);
        const gp = polar(gr, ga);
        nodes.push({ ...g, x: gp.x, y: gp.y, depth: 3, angle: ga, parentId: c.id });
...
function centerView() {
  ...
  transform.y = rect.height / 2 - (bounds.minY + bounds.height / 2) * transform.k;
  applyTransform();
}
...
function nodeLayout(n) {
  ...
  const w = Math.max(baseW, Math.min(isMajor ? 310 : 280, Math.ceil(longest * charWidth + 54)));
...
function prepareLayout(nodes) {
  nodes.forEach(n => { n._layout = null; nodeLayout(n); });
}
...
function resolveOverlaps(nodes) {
  ...
  for (const n of movable) {
    const { w, h } = nodeLayout(n);
    const minDistance = CORE_RADIUS + Math.max(w, h) / 2 + (n.depth === 1 ? 58 : 34);
...
function shouldShowCrossLink(a, b) {
  const touchesSelected = selectedId !== 'root' && (a === selectedId || b === selectedId);
...
function cycleLinkMode() {
  linkMode = linkMode === 'all' ? 'selected' : linkMode === 'selected' ? 'off' : 'all';
...
function render() {
  ...
  const crossLabelG = document.createElementNS('http://www.w3.org/2000/svg', 'g');
  ...
  for (const [a, b, label] of crossLinks) {
    ...
    const mx = (s.x + t.x) / 2;
    const my = (s.y + t.y) / 2;
    p.setAttribute('class', isSelectedCrossLink(a, b) ? 'cross selectedCross' : 'cross');
    p.setAttribute('d', `M${s.x},${s.y} Q${mx * 1.07},${my * 1.07} ${t.x},${t.y}`);
    ...
    if (shouldShowCrossLabel(a, b)) {
      ...
      labelGroup.setAttribute('class', isSelectedCrossLink(a, b) ? 'crossLabelGroup selectedCrossLabel' : 'crossLabelGroup');
  ...
  for (const n of nodes.filter(n => n.id !== 'root')) {
    ...
    const hay = JSON.stringify({ node: orig, code: card || null }).toLowerCase();
    const isMatch = q && hay.includes(q);
    const layout = nodeLayout(n);
    const [w, h] = [layout.w, layout.h];

    const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    g.setAttribute('class', `node ${classFor(n)} ${n.id === selectedId ? 'selected' : ''} ${isMatch ? 'match' : ''} ${card ? 'hasCode' : ''}`);
    ...
    layout.lines.forEach((line, i) => {
      ...
      text.setAttribute('y', -8 + (i - (layout.lines.length - 1) / 2) * layout.lineHeight);
    ...
    if (transform.k >= 0.55) {
      ...
      small.textContent = ((card ? 'CODE - ' : '') + (n.badge || n.type || '')).toUpperCase();
      g.appendChild(small);
    }
...
function list(title, arr) {
  return arr && arr.length ? `<div class="section-title">${title}</div><ul>${arr.map(x => `<li>${x}</li>`).join('')}</ul>` : '';
}
...
function ensureCodeModal() {
  ...
  el.addEventListener('click', e => { if (e.target === el) closeCodePanel(); });
...
function expandAll() {
  allNodes().forEach(n => { n._collapsed = false; });
  render();
}

function coreView() {
  allNodes().forEach(n => { n._collapsed = true; });
  data._collapsed = false;
  selectedId = 'root';
  showDetails(data);
  centerView();
...
document.getElementById('resetView').onclick = () => { centerView(); render(); };
